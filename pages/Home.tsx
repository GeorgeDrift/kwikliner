      ...
      <section className="bg-blue-600 pt-20 pb-32 px-4 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto text-center relative z-10">
          <h1 className="text-5xl md:text-7xl font-black text-white mb-6 tracking-tight">
            KwikLiner <span className="text-blue-200">Marketplace</span>
          </h1>
          ...
          {featuredLoads.map((load) => (
            <div key={load.id} className="bg-white rounded-[40px] overflow-hidden shadow-xl border border-slate-50 hover:border-blue-300 transition-all group hover:-translate-y-2">
              <div className="h-56 overflow-hidden relative">
                ...
                <div className="absolute top-6 left-6 bg-blue-600 text-white px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] shadow-lg">
                  {load.id}
                </div>
              </div>
              <div className="p-8">
                ...
                <div className="flex flex-wrap gap-4 mb-6">
                  <span className="text-[10px] font-black text-slate-500 flex items-center bg-slate-50 px-3 py-1.5 rounded-xl"><Package className="h-3 w-3 mr-2 text-blue-500" /> {load.cargo}</span>
                  <span className="text-[10px] font-black text-slate-500 flex items-center bg-slate-50 px-3 py-1.5 rounded-xl"><Gavel className="h-3 w-3 mr-2 text-blue-500" /> {load.weight}</span>
                </div>
                <div className="pt-6 border-t border-slate-50 flex justify-between items-center">
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Estimated Price</p>
            ...
            {toolsAndEquipment.map((tool, i) => (
              <div key={i} className="bg-white rounded-[40px] p-3 shadow-lg border border-slate-100 hover:shadow-2xl transition-all cursor-pointer group flex flex-col">
                <div className="h-64 rounded-[32px] overflow-hidden mb-6 relative">
                   ...
                   <div className="absolute bottom-4 left-4 bg-white/90 backdrop-blur-md px-4 py-2 rounded-2xl text-[10px] font-black text-blue-600 uppercase tracking-widest shadow-sm">
                     {tool.category}
                   </div>
                </div>
