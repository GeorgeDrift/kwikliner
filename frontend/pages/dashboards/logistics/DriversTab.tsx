    ...
    return (
        <div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-500">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                <div>
                    <h3 className="text-3xl sm:text-4xl font-black text-slate-900 tracking-tighter">Driver Management</h3>
                    <p className="text-slate-500 font-medium mt-1 text-sm sm:text-base">Monitor and manage your driver roster.</p>
                </div>
                <button
                    className="w-full sm:w-auto bg-[#6366F1] text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-indigo-100 flex items-center justify-center gap-2 hover:scale-105 transition-all"
                >
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                <div className="bg-white p-4 sm:p-6 rounded-[24px] sm:rounded-[32px] border border-slate-100 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                        <h4 className="text-[10px] sm:text-xs font-black uppercase tracking-widest text-slate-400">Total Drivers</h4>
                        <Users className="text-blue-600 hidden sm:block" size={20} />
                    </div>
                    <p className="text-2xl sm:text-3xl font-black text-slate-900">{drivers.length}</p>
                </div>
                <div className="bg-white p-4 sm:p-6 rounded-[24px] sm:rounded-[32px] border border-slate-100 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                        <h4 className="text-[10px] sm:text-xs font-black uppercase tracking-widest text-slate-400">Available</h4>
                        <ShieldCheck className="text-green-600 hidden sm:block" size={20} />
                    </div>
                    <p className="text-2xl sm:text-3xl font-black text-slate-900">{drivers.filter(d => d.status === 'Available').length}</p>
                </div>
                <div className="bg-white p-4 sm:p-6 rounded-[24px] sm:rounded-[32px] border border-slate-100 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                        <h4 className="text-[10px] sm:text-xs font-black uppercase tracking-widest text-slate-400">Total Trips</h4>
                        <Activity className="text-indigo-600 hidden sm:block" size={20} />
                    </div>
                    <p className="text-2xl sm:text-3xl font-black text-slate-900">{drivers.reduce((sum, d) => sum + d.trips, 0)}</p>
                </div>
                <div className="bg-white p-4 sm:p-6 rounded-[24px] sm:rounded-[32px] border border-slate-100 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                        <h4 className="text-[10px] sm:text-xs font-black uppercase tracking-widest text-slate-400">On Job</h4>
                        <Activity className="text-orange-600 hidden sm:block" size={20} />
                    </div>
                    <p className="text-2xl sm:text-3xl font-black text-slate-900">{drivers.filter(d => d.status === 'On Job').length}</p>
                </div>
            </div>
