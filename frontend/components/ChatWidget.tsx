...
const ChatWidget: React.FC<ChatWidgetProps> = ({ user }) => {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-4 left-4 sm:bottom-8 sm:left-8 z-[110] bg-blue-600 text-white p-4 sm:p-5 rounded-full shadow-2xl hover:scale-110 active:scale-95 transition-all group flex items-center space-x-3 border-4 border-white"
      >
  ...
  return (
    <div
      className={`fixed bottom-4 left-4 right-4 sm:right-auto sm:bottom-8 sm:left-8 z-[110] bg-white rounded-[32px] sm:rounded-[40px] shadow-2xl border border-slate-100 flex flex-col transition-all duration-300 ${isMinimized ? 'h-16 sm:h-20 w-auto sm:w-80' : 'h-[500px] sm:h-[600px] w-auto sm:w-[400px]'
        }`}
    >
      {/* Header */}
            ...
            {messages.map((msg) => (
              ...
              >
                <div className={`flex items-end space-x-2 max-w-[85%] ${msg.sender === 'user' ? 'flex-row-reverse space-x-reverse' : 'flex-row'}`}>
                  <div className={`shrink-0 h-8 w-8 rounded-full flex items-center justify-center ${msg.sender === 'user' ? 'bg-slate-100 text-slate-500' : 'bg-blue-600 text-white'
                    }`}>
                    {msg.sender === 'user' ? <UserIcon size={14} /> : <Bot size={14} />}
                  </div>
                  <div className={`p-4 rounded-[24px] text-sm font-medium leading-relaxed ${msg.sender === 'user'
