...
const ShipperDashboard: React.FC<ShipperDashboardProps> = ({ user }) => {
   ...
   useEffect(() => {
      const loadShipments = async () => {
         if (user.id) {
            try {
               const data = await api.getShipperLoads(user.id);
               // Assuming API returns array, we filter into categories or backend does.
               // For now, put all in Active for demo if structure not known, or assume 'Active' prop.
               // Mocking the structure mapping if API returns flat list:
               if (Array.isArray(data)) {
                  setShipmentsData(prev => ({ ...prev, Active: data.map(d => ({ ...d, color: 'text-amber-600 bg-amber-50' })) }));
               }
   ...
   const handlePostShipment = async (e: React.FormEvent) => {
      ...
      try {
         const res = await api.postLoad(payload);
         const entry = {
            id: res.id || `#KW-${Math.floor(Math.random() * 900000)}`,
   ...
   const handleAcceptBid = async (driverName: string, amount: string, bidId?: string) => {
      ...
      try {
         // If we have a bidId and API support, call it.
         if (bidId) await api.acceptBid(selectedLoadForBids.id, bidId);

         const updatedActive = shipmentsData.Active.map(s =>
            s.id === selectedLoadForBids.id
               ? { ...s, status: 'Waiting for Driver Commitment', price: amount, assignedDriver: driverName, depositStatus: 'Pending', color: 'text-amber-600 bg-amber-50' }
               : s
         );
         setShipmentsData(prev => ({ ...prev, Active: updatedActive }));
         alert(`Bid accepted! Notification sent to ${driverName}.`);
   ...
   // Debug Helper: Force Driver Confirmation
   const handleDebugDriverConfirm = () => {
      const waitingShipment = shipmentsData.Active.find(s => s.status === 'Waiting for Driver Commitment');
      if (waitingShipment) {
         setShipmentsData(prev => ({
            ...prev,
            Active: prev.Active.map(s =>
               s.id === waitingShipment.id
                  ? { ...s, status: s.paymentTiming === 'Full on Delivery' ? 'Ready for Pickup' : 'Pending Deposit' }
                  : s
            )
         }));
         alert(`[DEBUG] Driver ${waitingShipment.assignedDriver} commitment triggered manually.`);
      }
   };

   // Simulation: Driver confirms after 3 seconds
   useEffect(() => {
      const waitingShipment = shipmentsData.Active.find(s => s.status === 'Waiting for Driver Commitment');
      if (waitingShipment) {
   ...
   const handleProcessPayment = () => {
      ...
      const updatedActive = shipmentsData.Active.map(s => {
         if (s.id === activePaymentShipmentId) {
            return {
               ...s,
      ...
      alert(`Deposit processed via ${paymentMethod === 'BANK' ? 'Bank Transfer' : 'Mobile Money'}! Driver notified to pickup.`);
   };
